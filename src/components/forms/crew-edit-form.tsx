...
import { updatePerson } from "@/actions/crew";
...
import { crewFieldsFromPerson, type CrewFormFields } from "@/lib/vcard";
import {
  CrewSensitiveSection,
  type CrewSensitiveMaskedProps,
} from "@/components/crew/crew-sensitive-section";

const CREW_PROFILE_FORM_ID = "crew-profile-form";
...
export function CrewEditForm({
  ...
  person: PersonClient;
  sensitiveInitial: CrewSensitiveMaskedProps;
}) {
  const [state, action, pending] = useActionState(
    updatePerson.bind(null, person.id),
    null,
  );
  const [fields, setFields] = useState<CrewFormFields>(() =>
    crewFieldsFromPerson(person),
  );

  return (
    <div className="max-w-lg space-y-8">
    <form id={CREW_PROFILE_FORM_ID} action={action} className="space-y-8">
      {state?.error ? (
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <Button type="submit" form={CREW_PROFILE_FORM_ID} disabled={pending}>
            {pending ? "Lagrer…" : "Lagre endringer"}
          </Button>
          <Button type="button" variant="outline" asChild>
            <Link href="/crew">Tilbake</Link>
          </Button>
        </div>
      </div>
    </div>
